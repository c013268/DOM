...
{{ config(
    ...
    post_hook=["{{ fl_utils.m_mysql_process_deletions_from_silver_nonframework_using_model_id(  var('p_pipeline_name'), 'dom_silver', source('ord_mgmt_bronze', 'ord_cancel_line_tax_detail_archive_v'), this, 'src_load_ts', fl_utils.m_get_batch_id( var('p_pipeline_name') ) ) }}",
...
with cancel_line_tax_detail_stg as (
	select 
		src.pk
		,src.order_line_pk
		,src.org_id
		,src.header_tax_detail_id
		,src.tax_engine_id
		,src.tax_detail_id
		...
		,src.tax_type_id
		,src.context_id
		,src.is_retail_delivery_fee
		,src.is_invoice_tax
		,src.is_informational
		,src.rate_classification
		,src.vat_tax_code
		,src.tax_identifier3
		,src.tax_amount
		,src.tax_identifier4
	...
	from
		{{ source('ord_mgmt_bronze', 'ord_cancel_line_tax_detail_archive_v') }} src
	where
...
from cancel_line_tax_detail_stg src
where src.rnk=1