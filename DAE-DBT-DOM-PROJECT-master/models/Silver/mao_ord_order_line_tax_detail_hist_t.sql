...
{{ config(
    ...
    post_hook=["{{ fl_utils.m_mysql_process_deletions_from_silver_nonframework_using_model_id(  var('p_pipeline_name'), 'dom_silver', source('ord_mgmt_bronze', 'ord_order_line_tax_detail_archive_v'), this, 'src_load_ts', fl_utils.m_get_batch_id( var('p_pipeline_name') ) ) }}",
...
with order_line_tax_detail_stg as (
	select 
		src.pk
		,src.order_line_pk
		,src.org_id
	...
	from
		{{ source('ord_mgmt_bronze', 'ord_order_line_tax_detail_archive_v') }} src
	where
...
from order_line_tax_detail_stg src
where src.rnk=1
