...
{{ config(
    ...
    tags=['atp', 'network_alert']
) }}

with network_alert_v_dedup as (
    select
        ...
        header_object_state as transaction_type,
        item_id,
        null as on_hand_quantity,
        null as on_hand_status,
        ...
        null as future_quantity,
        view_name as selling_channel,
        null as first_available_future_qty,
        null as next_availability_date,
    ...
    from {{ source('src_atp_bronze', 'MAO_NW_AVAILABILITY_ALERT_V') }}
    where lower(header_message_type) = 'networkavailabilityalert'
    {% if is_incremental() %}
