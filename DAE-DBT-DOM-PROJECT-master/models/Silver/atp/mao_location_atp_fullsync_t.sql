...
{{ config(
    ...
    tags=['atp', 'location_fullsync']
) }}

with loc_atp_fullsync as (
    select
    ...
    from {{ source('src_atp_bronze', 'MAO_LOCATION_ATP_FULL_SYNC_V') }}
    where lower(header_message_type) = 'locationavailabilityfullsync' and location_location_id is not null
    {% if is_incremental() %}
        and kafka_timestamp::timestamp_ntz > {{ v_inc_load_ts }}
    {% endif %}
),
loc_atp_fullsync_dedup as (
    select
        *,
