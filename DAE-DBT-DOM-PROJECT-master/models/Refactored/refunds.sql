...
refunds AS (
    SELECT
        ...
        CAST(DATE(COALESCE(foh.confirmed_ts, foh.captured_ts)) AS VARCHAR) AS refund_header_date,
        CAST(CASE
            WHEN foh.pymt_status_id = '6000' THEN 'REFUND_PROCESSING'
            WHEN foh.pymt_status_id = '7000' THEN 'REFUNDED'
        ...
        CAST(CASE
            WHEN 
				foh.pymt_status_id = 5000 AND (fol.max_ln_total != fol.min_ln_total AND foh.min_ord_total != 0) THEN ABS(COALESCE(fol.max_ln_total,0) - COALESCE(fol.min_ln_total,0))
            ELSE 
        ...
        CAST(ABS(SUM(CASE
            ...
            ELSE 
				...
				ABS(COALESCE(fv.fv_cnlled_ord_shipping_tax_amt, 0) + COALESCE(fv.fv_ord_shipping_tax_amt, 0)) +
				ABS(COALESCE(fv.fv_cnlled_ord_sales_tax_amt, 0) + COALESCE(fv.fv_ord_sales_tax_amt, 0))
        END) OVER (PARTITION BY fol.org_id, fol.ord_id)) AS VARCHAR) AS ih_totalamount,
    ...
    LEFT JOIN payment_grouped pymt_line
        ON fol.ord_id = pymt_line.ord_id AND fol.org_id = pymt_line.org_id
