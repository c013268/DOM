...
mao_ord_fulfillment_detail AS (
    SELECT * FROM (SELECT fd.*, ROW_NUMBER() OVER (PARTITION BY fd.org_id, fd.ord_id, fd.ord_ln_id, fd.rel_id, fd.rel_ln_id ORDER BY fd.updated_ts DESC) AS ful_det_rnk FROM {{ source('dom_gold', 'fct_mao_ord_fulfillment_dtl_v') }} fd) 
    WHERE ful_det_rnk = 1
        {% if is_incremental() %}
