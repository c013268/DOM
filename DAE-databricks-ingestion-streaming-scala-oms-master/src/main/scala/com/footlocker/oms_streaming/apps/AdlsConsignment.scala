...
import com.footlocker.utils.Common.{applyschema,udfFunc,refreshPowerbi}
import com.footlocker.services.DeltaoptimizeService


object AdlsConsignment extends SnowflakeHelperService {
  ...
  def consignment_adls(oms_consignment_adls: DataFrame, spark : SparkSession,inputParams : Map[String, String]) {
    ...
    consignment_final.write.format("delta").mode("append").option("mergeSchema", "true").partitionBy("orderdate").save(inputParams("adls_url")+"/landing/sales/oms/consignments/")
    ...
    oms_consignment_adls.sparkSession.sql(s"""
          MERGE INTO sales.oms_consignments t
