...
object AdlsOrders extends SnowflakeHelperService {
    ...
    def AdlsOrdersFunc(oms_order_adls: DataFrame, spark: SparkSession, inputParams : Map[String, String],products: DataFrame) {
        ...
        order_pii.write.format("delta").mode("append").option("mergeSchema", "true").partitionBy("order_date").save(inputParams("adls_url")+"/landing/sales/oms/orders")


        val orderslinesquery = Source.fromInputStream(AdlsOrders.getClass.getResourceAsStream("/sqls/orderline.sql")).mkString
        ...
        val oms_customers = oms_order_adls.sparkSession.sql("""select company_number,
                        first_name,
                        last_name,
        ...
        oms_order_adls.sparkSession.sql(
            """Merge into customer_landing.oms_customers t
        ...
        //    val mergequery = Source.fromInputStream(AdlsOrders.getClass.getResourceAsStream("/sqls/mergesnoworderline.sql")).mkString
        ...
        println("triggering refresh in orders ")
        // ------------------ PowerBi refresh------------------------------------------
